      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest opacity-60">Teoría de Ciclos de Mercado y Análisis Técnico</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="relative">
            <button
              ...
              className="bg-surface-container-high border border-outline-variant/10 rounded-xl px-4 py-3 text-[10px] font-black uppercase tracking-widest text-on-surface flex items-center gap-3 min-w-[180px]"
            >
            ...
            {isSearchOpen && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-surface-container-high border border-outline-variant/10 rounded-xl shadow-2xl z-50 max-h-60 overflow-y-auto p-2">
                <div className="p-2 border-b border-outline-variant/10 mb-2 flex items-center gap-2">
                  ...
                  <input 
                    className="bg-transparent border-none focus:ring-0 text-[10px] font-black uppercase w-full"
                    placeholder="Buscar..."
                    value={searchQuery}
                ...
                {filteredAssets.map(asset => (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 rounded-lg text-[9px] font-black uppercase hover:bg-primary/10 flex items-center gap-3"
                  >
          ...
          <select 
            ...
            className="bg-surface-container-high border border-outline-variant/10 rounded-xl px-4 py-3 text-[10px] font-black uppercase tracking-widest text-on-surface outline-none"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-6">
          <div className="trading-card p-0 h-[450px] relative overflow-hidden">
            ...
            <div className="absolute top-6 left-6 z-10">
              <div className="bg-primary/10 backdrop-blur-md border border-primary/20 px-4 py-2 rounded-xl">
                <span className="text-[10px] font-black uppercase tracking-widest text-primary">Fase Actual: {wyckoffPhase}</span>
              </div>
            </div>
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={data}>
                ...
                <YAxis domain={['auto', 'auto']} orientation="right" tick={{ fontSize: 9, fill: '#666' }} axisLine={false} tickLine={false} />
                <Tooltip contentStyle={{ backgroundColor: '#0b0f14', border: 'none', borderRadius: '12px' }} />
                
                {wyckoffPhase.toLowerCase().includes("acumul") && (
                  <ReferenceArea y1={data[0]?.low} y2={data[0]?.high} fill="#00ffa3" fillOpacity={0.05} />
                )}
                
                <Line type="monotone" dataKey="close" stroke="#00ffa3" strokeWidth={2} dot={false} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="trading-card p-6 space-y-4">
              ...
              <p className="text-xs text-on-surface-variant leading-relaxed font-medium">{wyckoffExplanation}</p>
            </div>
            <div className="trading-card p-6 space-y-4 border-primary/20 bg-primary/5">
              <h3 className="text-[11px] font-black uppercase tracking-widest text-primary flex items-center gap-2">
                <Target className="w-4 h-4" /> Recomendación de Entrada
              </h3>
              <p className="text-xs text-on-surface font-black leading-relaxed">{recommendation}</p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="trading-card p-6 space-y-6">
            <h3 className="text-[11px] font-black uppercase tracking-widest text-on-surface">Indicadores Técnicos</h3>
            <div className="grid grid-cols-2 gap-3">
              {indicators.map(ind => (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-2 px-3 py-2 rounded-xl border text-[9px] font-black uppercase tracking-widest transition-all",
          ...
          <div className="space-y-4 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
            {indicators.filter(i => i.enabled).map(ind => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[9px] font-black uppercase tracking-widest text-primary">{ind.name}</span>
            ...
            {indicators.filter(i => i.enabled).length === 0 && (
              <div className="text-center p-8 border-2 border-dashed border-outline-variant/10 rounded-2xl opacity-30">
                <p className="text-[9px] font-black uppercase tracking-widest">Activa indicadores para ver el análisis</p>
              </div>
            )}
          </div>
        </div>
      ...
      >
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex items-center gap-4">
            ...
            <h3 className="text-lg font-black uppercase tracking-tighter text-on-surface">Conclusión y Recomendación Final</h3>
          </div>
          <div className="p-6 bg-surface-container-low/50 rounded-2xl border border-outline-variant/10">
            <p className="text-sm text-on-surface font-bold leading-relaxed italic">"{finalConclusion}"</p>
          </div>
        </div>
      </motion.div>
    </div>
